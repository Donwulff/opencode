...
2. **Launch up to 3 explore agents IN PARALLEL** (single message, multiple tool calls) to efficiently explore the codebase.
3. Read relevant files, review call paths, and gather concrete implementation context
4. Use question tool to clarify any remaining questions with the user

### Phase 2: Plan Development
Goal: Create one or more candidate plans for the user's request.
...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
      ...
      const resolveTools = Effect.fn("SessionPrompt.resolveTools")(function* (input: {
        ...
        const context = (args: any, options: ToolExecutionOptions): Tool.Context => ({
          ...
          extra: { model: input.model, bypassAgentCheck: input.bypassAgentCheck, command, promptOps },
      ...
      const command = Effect.fn("SessionPrompt.command")(function* (input: CommandInput) {
        ...
        if (cmd.name === Command.Default.PROMPT) {
          const templateParts = yield* resolvePromptParts(templateCommand)
          const parts = [...templateParts, ...(input.parts ?? [])]
          const cfg = yield* Effect.promise(() => Config.get())
          let agentName = input.agent ?? (yield* agents.defaultAgent())
          let modelRef = input.model ? Provider.parseModel(input.model) : yield* lastModel(input.sessionID)

          if (args[0]) {
            const name = args[0].startsWith("@") ? args[0].slice(1) : args[0]
            const resolved = yield* agents.get(name)
            if (resolved) {
              agentName = name
              args.shift()
            }
          ...
          yield* plugin.trigger("experimental.chat.system.transform", { sessionID: input.sessionID, model }, { system })
          if (system.length === 0) system.push(...original)
          ...
          const output = {
            ...
            codex_instructions: codex ? promptHeader : undefined,
            environment,
            instructions,
