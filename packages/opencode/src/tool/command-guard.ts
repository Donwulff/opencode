...
function cmd(ctx: Context) {
  return typeof ctx.extra?.command === "string" ? ctx.extra.command : undefined
}

export function assertLearnPath(ctx: Context, filePath: string) {
  const kind = cmd(ctx)
  if (kind === "review") {
...
export function assertLearnTask(ctx: Context) {
  if (cmd(ctx) !== "learn") return
  throw new Error("The /learn command cannot launch sub-agents via task tool.")
}

export function assertReadOnlyShell(ctx: Context, command: string) {
  const kind = cmd(ctx)
  if (!kind) return
