        ...
        for (const [id, provider] of configProviders) {
          ...
          yield* Effect.promise(async () => {
            try {
              ...
              if (!res.ok) {
                log.warn("autodiscover: failed to fetch models", { id, status: res.status })
                return
              }
              const data = await res.json()
              const modelList = data.data ?? data.models ?? []
              if (!Array.isArray(modelList) || modelList.length === 0) {
                log.warn("autodiscover: no models returned", { id })
                return
              }
              const existing = providers[providerID]
              const models: Record<string, Model> = existing?.models ?? {}
              for (const m of modelList) {
                ...
                models[modelID] = {
                  ...
                  options: {},
                  cost: { input: 0, output: 0, cache: { read: 0, write: 0 } },
                  limit: { context: contextLength, output: Math.min(contextLength, 16384) },
              ...
              log.info("autodiscover: found models", { id, count: Object.keys(models).length })
            } catch (e) {
              log.warn("autodiscover: error", { id, error: e })
            }
          })
