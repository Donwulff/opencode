        ...
        try {
          ...
          options["fetch"] = async (input: any, init?: BunFetchRequestInit) => {
            ...
            try {
              ...
              else if (typeof input === "object" && input && "url" in input) url = String((input as any).url)
              ...
              if (security) {
                const validation = validateUrlFromConfig(url, security)
                if (!validation.allowed) {
                  auditLogger.logToolRequest("unknown", "sdk-fetch", url, model.providerID, false, validation.reason)
                  throw new Error(validation.reason)
                }
                auditLogger.logToolRequest("unknown", "sdk-fetch", url, model.providerID, true, "URL validated")
              }
            } catch (err) {
              auditLogger.logToolRequest(
                ...
                model.providerID,
                false,
                `Failed to extract/validate URL: ${err}`,
              )
              throw err
            }
