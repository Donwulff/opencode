...
describe("V2 configuration loading", () => {
  for (const extension of ["json", "jsonc"]) {
    it.instance(`preserves fork configuration when loading V2 ${extension}`, () =>
      Effect.gen(function* () {
        const instance = yield* TestInstance
        const fs = yield* FSUtil.Service
        const file = path.join(instance.directory, `opencode.${extension}`)
        const provenance = { enabled: false, preview_chars: 64 }
        const network = { egress: { default_policy: "drop" as const } }
        yield* fs.writeWithDirs(file, JSON.stringify({ snapshots: false, provenance, network }))
        const config = yield* Config.use.get()
        expect(config.snapshot).toBe(false)
        expect(config.provenance).toEqual(provenance)
        expect(config.network).toEqual(network)
        const saved = ConfigParse.jsonc(yield* fs.readFileString(file), file)
        expect(saved).toMatchObject({ snapshots: false, provenance, network })
      }),
    )
  }
