...
You can also set a provider-level system prompt that is used when an agent does not define its own prompt:
...
```json title="opencode.json"
{
  ...
  "provider": {
    "local": {
      "system_prompt": "{file:.opencode/llama4.txt}"
    }
  }
}
```

`system_prompt` is applied as the base system prompt for that provider unless an agent prompt is explicitly set.
...
Plan agents can override the plan mode reminder text with `plan_reminder`:
...
```json title="opencode.json"
{
  ...
  "agent": {
    "plan": {
      "plan_reminder": "{file:.opencode/plan-reminder.txt}"
    }
  }
}
```

`plan_reminder` supports `{plan}` and `{plan_hint}` placeholders so the reminder can include the plan path or the default plan guidance text.
