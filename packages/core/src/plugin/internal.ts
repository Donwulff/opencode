  ...
  Effect.gen(function* () {
    ...
    yield* State.batch(
      Effect.gen(function* () {
        ...
        // Registered last so its catalog.transform runs after the provider/config plugins that
        // enable providers, giving the restriction filter the final say on `disabled`.
